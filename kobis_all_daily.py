...
OUT_DAY = os.path.join(BASE, "all_daily.csv")
OUT_META = os.path.join(BASE, "all_meta.csv")
LOG = os.path.join(BASE, "all_daily.log")
...
DAY_HEADER = ["날짜", "영화명", "개봉일", "관객수", "스크린수", "상영횟수",
              "좌석수", "좌석판매율", "좌석점유율"]
SEAT_CACHE = os.path.join(BASE, "all_seats.csv")
...
def seat_day(d):
    ...
    html = _download(PS, {
        ...
        "repNationCd": "", "wideareaCd": "", "sMovName": "", "sMovLang": "ko",
    })
    rows = _table(html)
    hi = next((i for i, c in enumerate(rows) if c and c[0] == "순위"), None)
    if hi is None:
