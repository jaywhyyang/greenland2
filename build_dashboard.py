...
# 개봉 N일차 종료 시점의 '누적 관객 ÷ 최종 총관객' 통상 비율.
...
# 너자2 실측 곡선 기준이되, 후반부(D12+)는 너자2 데이터가 D15에서 절단돼 late-frac이 과다(→최종 과소추정)
# 하므로 상자속의양(풀데이터 D14+)과 블렌드해 보정(2026-07-13). D12 0.965→0.953 등.
LIFETIME_FRAC = {1: 0.171, 2: 0.270, 3: 0.332, 4: 0.466, 5: 0.589, 6: 0.706, 7: 0.737,
                 8: 0.780, 9: 0.812, 10: 0.852, 11: 0.901, 12: 0.953, 13: 0.973, 14: 0.985,
                 15: 0.991, 16: 0.995, 17: 0.998, 18: 1.0}
...
def precise_forecast():
    ...
    # 호프(나홍진) 7/15 개봉 → 3주차 편성 삭감. 배급 확인(2026-07-13): 메가박스(관객 39%) 완전 종영,
    # CGV(36%)는 영등포 타임스퀘어 1관만 잔존, 롯데(25%)만 대부분 유지. 생존 ≈ 롯25%+CGV~4% ≈ 30%.
    # 잔존관 집중 fill 감안해도 ×0.33.
...
def sixtyk_countdown():
    ...
    if pf:
        ...
        return ('  <div class="forecast" style="background:linear-gradient(135deg,#241f16 0%,#1a1d27 70%);border-color:#5a4a2f">'
                ...
                f'<b style="color:#f4c89a">6만은 미달 확정적</b>(3주차 호프 개봉으로 메가박스(관객 39%) 완전 종영·CGV는 영등포만 잔존, 티프 미집행). '
...
def lifetime_forecast(cumul, completed_days):
    ...
    # 유사작 실측 곡선 기반이라 범위를 좁힘(하방=상자에 수렴 / 상단=프로모·주말 재가속)
    return {"cumul": int(cumul), "days": completed_days, "frac": f,
            "mid": r(cumul / f), "low": r(cumul / (f * 1.12)), "high": r(cumul / (f * 0.95))}
